...
<style>
  ...
  .badge-dpw    { background: #2e1a1a; color: #ef5350; border: 1px solid #ef5350; }
  ...
  .athlete-tour { font-size: 0.8rem; color: #888; margin-top: 0.25rem; }
...
TOUR_COLORS = {
    "LIV Golf":        "#4fc3f7",
    "PGA Tour":        "#66bb6a",
    "Korn Ferry Tour": "#ffa726",
    "DP World Tour":   "#ef5350",
}

def badge(tour):
    colors = {"LIV Golf": "liv", "PGA Tour": "pga", "Korn Ferry Tour": "kft"}
    cls = colors.get(tour, "other")
...
def fuzzy_match(name, player_list, threshold=70):
    ...
    for p in player_list:
        score = fuzz.token_sort_ratio(strip_accents(name.lower()), strip_accents(p.lower()))
        if score > best:
...
def build_athlete_data(mexican_golfers):
    """Build full schedule data for all athletes."""
    athletes = {}
    for name in mexican_golfers:
        athletes[name] = {"events": [], "tour": "Unknown"}
    ...
    for tour_code, tour_name in [("R", "PGA Tour"), ("S", "Korn Ferry Tour")]:
        ...
        for t in all_events:
            ...
            if not field:
                continue
            for name in mexican_golfers:
                matched, score, _ = fuzzy_match(name, field, threshold=70)
                if matched:
...
with st.sidebar:
    ...
    st.markdown("**Your Athletes**")
    default_athletes = """Abraham Ancer
Carlos Ortiz
Álvaro Ortiz
Rodolfo Cazaubon
...
Santiago de La Fuente"""
    ...
    st.markdown("---")
    st.caption("Data: PGA Tour GraphQL API (live) + LIV Golf (hardcoded 2026)")
...
# Top metrics
total_events = sum(len(v["events"]) for v in athlete_data.values())
upcoming_events = sum(
...
with tab1:
    if not filtered_athletes:
        ...
        for name in filtered_athletes:
            ...
            # Skip if tour filter excludes this athlete
            tour = data["tour"]
            ...
            with st.expander(f"{'🟢' if upcoming else '⚪'} {name}  —  {tour}", expanded=bool(upcoming)):
                ...
                if upcoming:
                    ...
                    for e in upcoming:
                        pill = f'<span class="upcoming-pill">upcoming</span>'
                        st.markdown(f"""
...
with tab2:
    # Flatten all upcoming events across all athletes
    all_upcoming = []
    for name in MEXICAN_GOLFERS:
    ...
    if not all_upcoming:
        ...
        <div class="no-results">
          No upcoming events found in the selected time range.<br>
          <small>Fields are posted Tuesday of event week. Try extending the time range.</small>
        </div>""", unsafe_allow_html=True)
    else:
        # Group by date
                  ...
                  <div style="display:flex;justify-content:space-between;align-items:center">
                    <div>
                      ...
                      <p class="event-meta">
                        ...
                        📍 {e['location']} &nbsp;|&nbsp;
                        {badge(e['tour'])}
                        {"&nbsp;|&nbsp; 💰 " + e['purse'] if e.get('purse') and e['purse'] not in ['N/A', '$0', ''] else ""}
                      </p>
                    </div>
                  </div>
                </div>""", unsafe_allow_html=True)
...
with tab3:
    ...
    with sub1:
        ...
        all_pga = sorted(upcoming_pga + completed_pga, key=lambda x: x["startDate"])
        for t in all_pga:
            date = ts_to_date(t["startDate"])
            is_up = date >= today
            pill = '<span class="upcoming-pill">upcoming</span>' if is_up else '<span class="past-pill">completed</span>'
            st.markdown(f"""
            <div class="event-card" style="{'opacity:1' if is_up else 'opacity:0.4'}">
              <div style="display:flex;justify-content:space-between;align-items:center">
                <div>
    ...
    with sub2:
        ...
        all_kft = sorted(upcoming_kft + completed_kft, key=lambda x: x["startDate"])
        for t in all_kft:
            date = ts_to_date(t["startDate"])
            is_up = date >= today
            pill = '<span class="upcoming-pill">upcoming</span>' if is_up else '<span class="past-pill">completed</span>'
