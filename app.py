...
<style>
  ...
  .event-card { background: #141414; border: 1px solid #222; border-left: 4px solid #006847; border-radius: 10px; padding: 1rem 1.25rem; margin-bottom: 0.75rem; transition: border-color 0.2s; }
...
def fuzzy_match(name, player_list, threshold=80):
    ...
    if len(parts) >= 2:
        ...
        if len(parts) >= 3:
            ...
            variants.append(f"{last_name}, {first_only}")
    best, best_name = 0, ""
    for p in player_list:
        for variant in variants:
            score = fuzz.token_sort_ratio(
                strip_accents(variant.lower()),
...
def fetch_pga_field(tournament_id):
    ...
    {{
      field(id: "{tournament_id}") {{
        players {{
          ... on {player_type} {{ displayName country }}
        }}
      }}
    }}
    """
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                for entry in future.result():
                    ...
                    athletes[name]["events"].append(entry)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**🏌️ Athlete List** *(Mexican nationals only)*")
    st.caption("One name per line. You control this list.")
    ...
    st.caption("Confirms Mexican nationality via API. Slower to load.")
...
with tab2:
    ...
    if not all_upcoming:
        ...
        <div class="no-results">
          No upcoming events in selected time range.<br>
          <small>Fields post Tuesday of event week. Try extending weeks ahead.</small>
        </div>""", unsafe_allow_html=True)
    else:
